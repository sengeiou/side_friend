...
public class V8 extends V8Object {
    ...
    //by 刘逸凡
    void registerCallback(final JavaCallback callback, final long objectHandle, final String jsFunctionName,final V8Object object) {
        long methodID = registerJavaMethod(getV8RuntimePtr(), objectHandle, jsFunctionName, false);
        createAndRegisterMethodDescriptor(callback, methodID,object);
    }
    ...
    //by 刘逸凡
    void createAndRegisterMethodDescriptor(final JavaCallback callback, final long methodID,V8Object object) {
        MethodDescriptor methodDescriptor = new MethodDescriptor();
        methodDescriptor.callback = callback;
        methodDescriptor.object = object;
        functionRegistry.put(methodID, methodDescriptor);
...
//            Iterator<Map.Entry<Long, MethodDescriptor>> it = functionRegistry.entrySet().iterator();
//            while (it.hasNext()) {
//                Map.Entry<Long, MethodDescriptor> entry = it.next();
//                if (entry.getValue().object == reference) {
//                    releaseMethodDescriptor(v8RuntimePtr,entry.getKey());
//                    it.remove();
//                }
//            }
//        }

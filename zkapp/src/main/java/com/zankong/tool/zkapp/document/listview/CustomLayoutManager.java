...
public class CustomLayoutManager extends RecyclerView.LayoutManager {
    ...
    public CustomLayoutManager(ZKActivity activity){
        mActivity = activity;
    }
    ...
    private void calculateChildrenSite(RecyclerView.Recycler recycler) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            addView(view);
            // 我们自己指定ItemView的尺寸。
            measureChildWithMargins(view, mActivity.width / 2, 0);
            ...
            if (i % 2 == 0) { // 当i能被2整除时，是左，否则是右。
                // 左
                mTmpRect.set(0, totalHeight, mActivity.width / 2, totalHeight + height);
            } else {
                // 右，需要换行
                mTmpRect.set(mActivity.width / 2, totalHeight, mActivity.width,
                        totalHeight + height);
    ...
    private void recycleAndFillView(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            if (Rect.intersects(displayRect, allItemRects.get(i))) {
                ...
                measureChildWithMargins(itemView, mActivity.width / 2, 0);
